...
public class BoardDao implements Board {
    ...
    public int modifyInquiry(Inquiry inquiry) {
        ...
        try {
            conn = DBUtil.getConnection();
            String sql = "{call modifyInquiry(?, ?, ?, ?)}";
            cstmt = conn.prepareCall(sql);

            cstmt.setInt(1, inquiry.getIqIdx());
            cstmt.setString(2, Character.toString(inquiry.getIqType()));
            cstmt.setString(3, inquiry.getIqTitle());
            cstmt.setString(4, inquiry.getIqContent());
    ...
    public int addResponse(Response response) {
        ...
        try {
            conn = DBUtil.getConnection();
            String sql = "{call addResponse(?, ?, ?)}";
            cstmt = conn.prepareCall(sql);

            cstmt.setInt(1, response.getTaIdx());
            cstmt.setInt(2, response.getIqIdx());
            cstmt.setString(3, response.getRContent());
    ...
    public int modifyResponse(Response response) {
        ...
        try {
            conn = DBUtil.getConnection();
            String sql = "{call modifyResponse(?, ?)}";
            cstmt = conn.prepareCall(sql);

            cstmt.setInt(1, response.getIqIdx());
            cstmt.setString(2, response.getRContent());
